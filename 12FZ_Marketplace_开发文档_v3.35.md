# 12FZ Marketplace 开发文档 v3.35

> AI原生 · 业务全Go · 日均10万单 · 2026-08-13（v3.35 08-15：合并用户体系规则 v1.6）

---

## 零、v3.34 → v3.35 变化

| 模块 | v3.34 状态 | v3.35 状态 |
|------|:--:|:--:|
...
> **两域关系（v3.35 明确，详见《12FZ 用户体系统一规则 v1.6》）**：go.12fz.com 是唯一登录入口（签发统一 JWT）→ ai.12fz.com 本地验签；商户在 go 开账户/充值/下单，在 ai 用 `sh-` key 消费模型，账本/余额同一 org 名下；设备用 `zc-` 注册码贯穿两域（归属规则 8）。
...
> 背景：商户隔离是**三个维度**的，漏任一维即越权。v2.0 只做了 org_id（订单接口），v2.1 补齐 industry_code（行业授权）+ menu_ids（菜单授权）。曾因只做 org_id 漏 industry_code，导致商户看到全部 15 个行业。
...
1. **org_id 只信服务端解析**：`Authorization: Bearer session-<uid>` → 查 `org_user` 表得权威 org_id；禁止信任 `X-Org-ID`/`X-Role`/`X-User-ID` header、query、body。
...
- [ ] 这个接口的数据要不要按商户过滤？
- [ ] 三个维度都查了吗？（org_id + industry_code + menu_ids）
- [ ] org_id 从服务端 session 解析（`middleware.RequireOrg` fail-closed），不信任客户端传参？
...
| L2 handler | 三维度过滤：`ScopeFrom` + 列表强制 `AND org_id`/`industry_code` | ✅ |
...
**本次已改文件**：
- org_id 维度（commit 3b94bee）：`internal/order/handler.go`、`internal/mall/handler.go`、`internal/cashier/handler.go`、`internal/middleware/middleware.go`。验证：不带 X-Org-ID → 401。
...
### 3.3 主机/设备 org 归属（v3.35 新增·规则 8/8.5）

> 曾因按主机名/IP 推断归属导致误改（qiuming/gungzhou 被误判为 M_QIU 测试商户）。归属是数据决策，**禁止推断**。

1. **新设备唯一合法途径**：生成注册码（`zc-`，强制 org_id，空 org 直接 400）→ `RegisterDevice` 兑换 `host_reg_codes.org_id` 写入 `hosts.org_id`
...
- 现象：RLS 启用后 chat-token（app_zhongtai 连接未设 org_id）查账本被过滤 → 余额 0 → **全员 402**
...
```
chat.contacts       (user_id, contact_id)  — 人↔人
chat.user_agents    (user_id, agent_id)    — 人↔Agent，FK 约束
chat.user_devices   (user_id, device_id)   — 人↔主机，FK 约束
```
...
| 版本 | 日期 | 内容 |
|---|------|------|
| **v3.35** | 08-15 | 明确两域关系（合并用户体系规则 v1.6）+ 计费价格修正（v4-flash/pro 官方×2）+ 主机归属规则 8/8.5（审计+漂移检测）+ RLS 402 故障修复记录 |
| **v3.34** | 08-13 | 商户数据隔离规范 v2.1（三维度：org_id+行业+菜单）+ 三层防线（规范+工具+测试）+ 三层防御 L1/L2/L3（41表 RLS）+ 订单接口 org_id + 行业授权过滤 + 中转站计费防护 + 旧 key 清理 |
